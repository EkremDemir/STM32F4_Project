...
int main(void)
{
	...
	while(1)
	{
		...
		if(rx_buffer[0] == 'o' && rx_buffer[1] == 'n')
		{
			...
			if(led_state != true)
			{
				HAL_UART_Transmit(&huart2,(uint8_t *)info1_buffer, 50, 500);
				led_state = true;
			}
			else
			{
				HAL_UART_Transmit(&huart2,(uint8_t *)info3_buffer, 50, 500);
			}
			rx_buffer[0] =0;
			rx_buffer[1] =0;
		...
		else if(rx_buffer[0] == 'o' && rx_buffer[1] == 'f' && rx_buffer[2] == 'f')
		{
			...
			if(led_state != false)
			{
				HAL_UART_Transmit(&huart2,(uint8_t *)info2_buffer, 50, 500);
				led_state = false;
			}
			else
			{
				HAL_UART_Transmit(&huart2,(uint8_t *)info4_buffer, 50, 500);
